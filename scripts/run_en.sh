    ...
    if [[ -n "${REVISIONS_MAP[$MODEL]}" ]]; then
        ...
        for REVISION in ${REVISIONS_MAP[$MODEL]}; do
            ...
            lm_eval --model vllm \
                ...
                --include_path ../../repo_from_others/zh_qa/FinBen/tasks/chinese
        ...
        lm_eval --model vllm \
            --model_args "pretrained=$MODEL,tensor_parallel_size=1,gpu_memory_utilization=0.8,max_model_len=1024,trust_remote_code=True" \
            --tasks reg-qa \
            --batch_size auto \
            --output_path ../results \
            --hf_hub_log_args "hub_results_org=TheFinAI,details_repo_name=lm-eval-results-zh-5shot,push_results_to_hub=True,push_samples_to_hub=False,public_repo=False" \
...
# zh-fineval,zh-reg-classification,reg-qa
